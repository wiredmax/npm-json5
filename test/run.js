...
var npmcli = path.resolve(npmpkg, 'npm-json5.js')
...
function setup (cb) {
  cleanup(function (er) {
    if (er) return cb(er)
    console.log('node \'' + npmcli + '\' install \'' + npmpkg + '\'');
    ...
    chain(
      [
        ...
        [execChain, packages.map(function (p) {
          return [ npm_exec + ' install packages/' + p, testdir ]
        })],
        [execChain, packages.map(function (p) {
          return [ npm_exec + ' test -ddd', path.resolve(base, p) ]
        })],
        [execChain, packagesToRm.map(function (p) {
          return [ npm_exec + ' rm ' + p, root ]
        })],
        installAndTestEach
      ],
      cb
  ...
  function installAndTestEach (cb) {
    ...
    var thingsToChain = [
        ...
        [execChain, flatten(packages.map(function (p) {
          return [
            [npm_exec + ' install packages/' + p, testdir],
            [npm_exec + ' test', path.resolve(base, p)],
            [npm_exec + ' rm ' + p, root]
          ]
        }))]
      ]
    if (process.platform !== 'win32') {
      // Windows can't handle npm rm npm due to file-in-use issues.
      thingsToChain.push([exec, npm_exec + ' rm npm', testdir])
    }
